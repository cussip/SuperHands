...
<body>
	...
	${notice.title} <br>
	<%-- No48_ELstorage
		데이터 저장소 4가지: page, request, session, application
		4가지 저장소에 동일한 키워드를 갖는 값을 입력하는 경우
		우선순위에 따라 page -> request -> session -> application 순서로 검색한다.
	--%>
	<%	//page: page내에서 저장소로 사용할 수 있는 객체
		pageContext.setAttribute("aa", "hello");
		pageContext.setAttribute("result", "page result HELLO!");
	%>
	${aa} <br>
	${result} <br> 				<%-- page 저장소의 result가 출력된다. --%>
	${requestScope.result} <br>	<%-- request 저장소의 result가 출력된다 --%>
	n = ${param.n} <br>
	${header.accept} <br>
	...
	<%-- 
		위 문장은 아래 문장과 의미상 동일하다. 
		EL에서는 getter메소드를 사용 가능하며, 호출할 때 위와 같이 사용한다
