...
public class MiniOmok extends JFrame implements MouseListener {
	...
	final	int	STON_SIZE		= (int)(LINE_WIDTH * 0.8);		// 돌의 크기
	...
	public void drawBoard(Graphics g) {
		...
		for(int i = 0; i < LINE_NUM; i++) {
			//g.drawLine(X0, Y0+i*LINE_WIDTH-Y0, BOARD_SIZE, Y0+i*LINE_WIDTH-Y0);	// 가로줄
			g.drawLine(X0, Y0+i*LINE_WIDTH, X0+BOARD_SIZE, Y0+i*LINE_WIDTH);//가로줄
			
			//g.drawLine(X0+i*LINE_WIDTH-X0, Y0, X0+i*LINE_WIDTH-X0, BOARD_SIZE);	// 세로줄
	...
	public void mousePressed(MouseEvent e) {
		...
		// 3. x 와 y 의 값에서 돌의 크기(STON_SIZE)의 절반을 빼야 클릭한 곳에 돌이 위치한다.
		// 4. 눌려진 버튼이 마우스 왼쪽 버튼이면 x,y위치에 검은 돌을 그리고,
				// myTime == false 인 경우에 검은 돌을 그린다.
		//    눌려진 버튼이 마우스 오른쪽 버튼이면 x,y위치에 흰 돌을 그린다.
				// myTime = true 인 경우에 희 돌을 그린다.
